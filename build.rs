...
fn main() {
	...
	cc::Build::new()
		.files(["repalette-sys/cli.c", "repalette-sys/repalette.c"])
		.std("c99")
		.opt_level(3)
		.flags(args.iter())
...
fn build_wasm(args: &[&str]) {
	...
	let status = Command::new(&clang)
		...
		.args(["-o", "studio/www/repalette.wasm"])
		.args(["repalette-sys/wasm.c", "repalette-sys/repalette.c"])
		.status()
		.unwrap_or_else(|e| panic!("failed to invoke `{clang}` for the wasm build: {e}"));

	assert!(
		status.success(),
		"wasm build failed: `{clang}` exited with {status}"
	);
}
